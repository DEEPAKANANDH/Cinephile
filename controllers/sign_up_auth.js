...
const signup_authentication=async (e)=>{
    e.preventDefault()
    try{
        const signin=await axios.post("http://localhost:3000/auth/signup_auth",{Username:username.value,Email:email.value,Password:password.value,Confirm_password:confirm_password.value})
